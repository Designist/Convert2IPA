...
/*
var rulefile = "test.rules";

var jsonFile = new XMLHttpRequest();
jsonFile.open("GET", rulefile, true);
jsonFile.send();
	...
	for (var i = 0; i < data.length; i++) {
		if (data[i].length > 0 && data[i][0] != "#") {
			...
			if (split_rule[0] != headers[0]) { // ignore header
				var rule_dict = {}
				for (var j = 0; j < headers.length; j++) {
					rule_dict[headers[j]] = split_rule[j].replace(/^["'`](.+)["'`]$/,'$1')
				}
	...
	translate(source) {
		...
		if (source in this.words) {
			...
			for (var i = 0; i < source_list.length; i++) {
				...
				if (this.matches.hasOwnProperty(sfrom)) {
					target_list.push(this.matches[sfrom])
				}
				else {
					var translations = []
					...
					if (translations.length > 0) {
						var translation = translations.sort(function(a, b) {
							return (b[0] - a[0])
						})[0][1]
			...
			ipa_translations = ipa_translations.sort(function(a, b) {
				return (b[0] - a[0])
			})
